...
static uint32_t * xxtea_to_long_array(const uint8_t * data, size_t len, int inc_len, size_t * out_len) {
    uint32_t i, n, *result;
    n = len >> 2;
    n = (((len & 3) == 0) ? n : n + 1);
    if (inc_len) {
        result = (uint32_t *) emalloc((n + 1) << 2);
        if(result == NULL)
        {
            return NULL;
        }
...
unsigned char *xxtea_to_byte_array(uint32_t *data, size_t len, int include_length, size_t *ret_len) {
    uint32_t i, n, m;
...
static unsigned char * xxtea_ubyte_decrypt(const uint8_t * data, size_t len, const uint8_t * key, size_t * out_len) {
    unsigned char *out;
    uint32_t *data_array, *key_array;
    size_t data_len, key_len;
    ...
    data_array = xxtea_to_long_array(data, len, 0, &data_len);
    key_array  = xxtea_to_long_array(key, 16, 0, &key_len);
    out = xxtea_to_byte_array(xxtea_uint_decrypt(data_array, data_len, key_array), data_len, 1, out_len);
